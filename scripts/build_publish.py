...
FALLBACK_SLUG = "containerteknik-for-chefer-i-offentlig-sektor"
...
def validate(root: Path, metadata: dict) -> tuple[list[Path], Path | None]:
    ...
    if not chapter_names:
        raise SystemExit("Metadata saknar kapitel")
    ...
    for index, rel in enumerate(chapter_names):
        ...
        if heading is None:
            ...
            match = re.match(r"^Kapitel\s+(\d+)\s*:\s*.+$", heading)
            if not match:
                errors.append(f"{rel}: första H1 måste följa 'Kapitel N: Titel'")
            elif int(match.group(1)) != index + 1:
                errors.append(f"{rel}: kapitelnumret måste vara {index + 1}")
...
def postprocess_epub(epub: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="containerteknik-chefer-epub-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="containerteknik-chefer-pdf-") as tmp_name:
        tmp = Path(tmp_name)
